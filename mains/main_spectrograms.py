...
def main(model, epochs=30):
    ...
    data_list, genres_list = LoadDataPipeline('spectrogram')
    ...
    #model = WaveNet(input_shape=(13,2581),num_classes=8)
    model = model
    ...
    for epoch in range(1, epochs + 1):
        ...
        if acc > accmax:
            ...
            accmax = acc
            y_pred = prediction
        spectrogram_accuracies.append(acc)
    return model, targets, probas, y_pred, accmax, train_dataloader, test_dataloader, spectrogram_accuracies


if __name__ == "__main__":
    model, targets, probas, y_pred, accmax, _ , _, _ = main(RNN('spectrogram'))
